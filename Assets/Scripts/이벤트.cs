...
{
    public Dictionary<int, EventData> 공부 = new Dictionary<int, EventData>();
    public Dictionary<int, EventData> 여가 = new Dictionary<int, EventData>();
    public Dictionary<int, EventData> 사랑 = new Dictionary<int, EventData>();
    public Dictionary<int, EventData> 돈 = new Dictionary<int, EventData>();
    ...
    private void Read(string path_name)
    {
        ...
        while (!end)
        {
            ...
            else if (!first)
            {
                ...
                공부.Add(key, new EventData(이름, 설명, 대사, 포만감, 피로도, 매력, 아드레날린, 인기, 주머니사정, 지식, 지지도, 시간, 합산));
            }
            else first = false;
        }
    }
